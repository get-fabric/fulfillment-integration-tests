...
get_not_ready_running_pods_count () {
    echo $((`kubectl --kubeconfig $kubeconigPath get pods --all-namespaces | grep -v "kube-system" | grep "Running|Pending" | grep 0/ | wc -l`))
}

list_not_ready_runninng_pods () {
    kubectl --kubeconfig $kubeconigPath get pods --all-namespaces | grep -v "kube-system" | grep "Running|Pending" | grep 0/1
}
